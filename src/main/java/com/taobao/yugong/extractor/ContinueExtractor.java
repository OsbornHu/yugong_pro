...
public class ContinueExtractor extends AbstractYuGongLifeCycle implements Runnable {
    ...
    public ContinueExtractor(OracleFullRecordExtractor oracleFullRecordExtractor, YuGongContext context,
    ...
    public void run() {
        while (running) {
            jdbcTemplate.execute(oracleFullRecordExtractor.getExtractSql(), (PreparedStatementCallback) ps -> {
                ps.setObject(1, id);
                ...
                while (rs.next()) {
                    ...
                    for (ColumnMeta pk : context.getTableMeta().getPrimaryKeys()) {
                        ColumnValue cv = oracleFullRecordExtractor.getColumnValue(rs, context.getSourceEncoding(), pk);
                        pks.add(cv);
            ...
            for (ColumnMeta col : context.getTableMeta().getColumns()) {
                ColumnValue cv = oracleFullRecordExtractor.getColumnValue(rs, context.getSourceEncoding(), col);
                cms.add(cv);
            }

            Record re = new Record(context.getTableMeta().getSchema(), context.getTableMeta().getName(), pks, cms);
